...
void main() {
  ...
  test(
    ...
    () async {
      ...
      expect(
        store.sales(cashierId: 'cashier-1').map((s) => s.offlineRequestId),
        ['a', 'b'],
      );
  ...
  test(
    'putSale replaces the same sale, e.g. when it is marked failed',
    () async {
  ...
  test(
    'keeps the offline shift and the cached server shift per cashier',
    () async {
